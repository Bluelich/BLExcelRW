...
@property (strong, nonatomic) NSMutableArray *arrayWorkSheet;
@end
...
- (instancetype)init
{
    ...
    if (self) 
    {
        _defaultStyle = [[RSStyle alloc] init];
        _defaultStyle.font = [UIFont systemFontOfSize:14];
        _defaultStyle.size = 14;
        _defaultStyle.color = [UIColor blackColor];
        _defaultStyle.alignmentH = RSStyleMiddleAlign;
        _defaultStyle.alignmentV = RSStyleCenterAlign;
        
        _version = @"1.0";
        _author = [[NSString alloc] initWithFormat:@"system"];
        _date = [NSDate date];
        
        _arrayWorkSheet = [[NSMutableArray alloc] init];
...
{
    [_arrayWorkSheet addObject:sheet];
}
-(NSArray *)workSheets
{
    return _arrayWorkSheet;
}
...
- (BOOL)writeWithName:(NSString*)name toPath:(NSString*)path
{
    ...
    BOOL dateType = NO;
    int i = 0;
    for (RSworkSheet * sheet in _arrayWorkSheet)
    {
        
        for (RSworkSheetRow * sheetRow in sheet.arrayWorkSheetRow) 
        {
            dateType = NO;
            if (![sheetRow.style isEqual:_defaultStyle])
            {
            ...
            {
                if (![cell.style isEqual:_defaultStyle])
                {
                    [styleDefault appendFormat:@"<Style ss:ID=\"s%i\">\n<Alignment ss:Vertical=\"%@\" ss:Horizontal=\"%@\"/>\n<Borders/>\n<Font ss:FontName=\"%@\" ss:Size=\"%.2f\" ss:Color=\"#%@\"/>\n<Interior/>\n<NumberFormat/>\n<Protection/>\n</Style>\n",
                     60+i,
                     [self formatTypeToStringVertical: sheetRow.style.alignmentV], 
                ...
                if ((cell.type == cellTypeDate)&&(![sheetRow.style isEqual:_defaultStyle]))
                {
    ...
    int j = 0;
    for (RSworkSheet * sheetwork in _arrayWorkSheet)
    {
        [sheet appendFormat:@"<Worksheet ss:Name=\"%@\">\n", sheetwork.name];
        
        [sheet appendFormat:@"<Table ss:ExpandedColumnCount=\"%d\" ss:ExpandedRowCount=\"%ld\" x:FullColumns=\"1\" x:FullRows=\"1\" ss:DefaultColumnWidth=\"%.2f\" ss:DefaultRowHeight=\"%.2f\">\n",[self countMaxCell:(NSMutableArray *)sheetwork.arrayWorkSheetRow],[sheetwork.arrayWorkSheetRow count],sheetwork.columnWidth,sheetwork.rowHeight];[sheet appendFormat:@"<Column ss:Width=\"80\"/>\n"];
